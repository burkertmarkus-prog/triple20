...
function loadActiveCompetition(){
  ...
  state.settings={...(state.settings||{}),competition:state.activeCompetition};
}
...
function renderAdminMembers(){
  ...
  const cards=profiles.map(profile=>{
    const ownProfile=profile.id===c.user?.id,role=c.memberRoles?.[profile.id]||'member',roleLabel=role==='admin'?'Administrator':role==='tournament_manager'?'Turnierleitung':'Mitglied',name=profile.display_name||(ownProfile?'Turnierleitung':'Name noch nicht eingetragen'),nickname=profile.nickname||(ownProfile?'Administrator':'Spitzname fehlt'),initial=esc((profile.nickname||profile.display_name||(ownProfile?'A':'?')).trim().charAt(0).toUpperCase()||'?'),photo=c.adminProfileAvatars?.[profile.id],avatar=photo?`<img src="${esc(photo)}" alt="">`:initial;
    ...
    const roleControl=isFullAdmin()?`<label class="member-role-control">Berechtigung<select data-member-role="${esc(profile.id)}" ${ownProfile?'disabled':''}><option value="member" ${role==='member'?'selected':''}>Mitglied</option><option value="tournament_manager" ${role==='tournament_manager'?'selected':''}>Turnierleitung</option><option value="admin" ${role==='admin'?'selected':''}>Administrator</option></select></label>`:`<small>${esc(roleLabel)}</small>`;
...
window.T20Cloud={
  ...
  async loadOwnRole(uid){
    try{const client=requireSupabaseClient(),{data,error}=await withTimeout(client.rpc('triple20_my_role'),10000,'Berechtigungsprüfung dauert zu lange.');if(error)throw error;const role=['admin','tournament_manager'].includes(data)?data:'member';if(role!=='member')localStorage.setItem('triple20_admin_uid',uid);else localStorage.removeItem('triple20_admin_uid');return role}
      ...
      // Rueckwaertskompatibel, bis das neue Rollen-SQL einmal ausgefuehrt wurde.
      try{const client=requireSupabaseClient(),{data,error:legacyError}=await withTimeout(client.from('triple20_admins').select('user_id').eq('user_id',uid).maybeSingle(),10000,'Adminprüfung dauert zu lange.');if(legacyError)throw legacyError;const role=data?.user_id===uid?'admin':'member';if(role==='admin')localStorage.setItem('triple20_admin_uid',uid);return role}
      catch(legacyError){console.warn('Berechtigungsprüfung fehlgeschlagen',error,legacyError);return localStorage.getItem('triple20_admin_uid')===uid?'admin':'member'}
    }
  },
...
function renderTournament(){
  ...
  if(!state.started){$('#setupSection').classList.remove('hidden');$('#tournamentSection').classList.add('hidden');$('#tournamentName').value=competitionEventName()||'Freitag-Abend-Cup';renderReadonlyMode();return}
...
function addTournamentToSeason(seasonId,tournament){
  if(!isClubMode())return null;
  const assigned=assignedSeasonForTournament(tournament),season=assigned||seasonStore.seasons.find(s=>s.id===seasonId);if(!season)return null;tournament.scheduledSeasonId=season.id;
...
function calculateSeasonStandings(season=selectedSeason()){
  ...
  return [...players.values()].map(player=>{
    ...
    const configuredDrops=season.dropCount||0,effectiveDrops=tournaments.length>configuredDrops?configuredDrops:0,dropped=calculateDropResults(entries,effectiveDrops),used=dropped.filter(e=>!e.dropped),played=entries.filter(e=>e.present),wins=entries.reduce((sum,e)=>sum+e.wins,0),losses=entries.reduce((sum,e)=>sum+e.losses,0),byes=entries.reduce((sum,e)=>sum+(e.byes||0),0),max180=entries.reduce((sum,e)=>sum+e.max180,0),checkout=Math.max(0,...entries.map(e=>e.checkout||0)),statEntries=entries.filter(e=>e.average!==null),statDarts=statEntries.reduce((sum,e)=>sum+(e.darts||0),0),average=statEntries.length?Number((statDarts?statEntries.reduce((sum,e)=>sum+e.average*(e.darts||0),0)/statDarts:statEntries.reduce((sum,e)=>sum+e.average,0)/statEntries.length).toFixed(1)):null,first9Entries=entries.filter(e=>e.first9!==null),first9=first9Entries.length?Number((first9Entries.reduce((sum,e)=>sum+e.first9,0)/first9Entries.length).toFixed(1)):null;
    return{name:player.name,profileId:player.profileId,totalPoints:entries.reduce((sum,e)=>sum+e.points,0),cleanPoints:used.reduce((sum,e)=>sum+e.points,0),played:played.length,wins,losses,byes,max180,checkout,average,first9,darts:statDarts,statMatches:entries.reduce((sum,e)=>sum+(e.statMatches||0),0),dropResults:dropped.filter(e=>e.dropped),entries:dropped,participation:tournaments.length?played.length/tournaments.length:0,winRate:wins+losses?wins/(wins+losses):0};
...
function renderSeasonImport(winner){
  ...
  const assigned=assignedSeasonForTournament(state),current=assigned||seasonForDate(todayIso())||seasons[0],stats=state.players.map(p=>{const auto=aggregateAutodartsStats(p);return `<div class="season-stat-row" data-stat-player="${esc(p)}"><b>${esc(p)}</b><label>180er<input type="number" min="0" value="${auto.max180}" data-stat-180></label><label>Höchstes Checkout<input type="number" min="0" max="170" value="${auto.highestFinish}" data-stat-checkout></label>${auto.matches?`<small>${auto.matches} Autodarts-Spiel${auto.matches===1?'':'e'} · Ø ${auto.average??'–'}</small>`:''}</div>`}).join('');
...
function renderSeasonStandings(season=selectedSeason(),rows=calculateSeasonStandings(season)){
  ...
  const desktop=`<div class="table-wrap season-desktop-ranking"><table><thead><tr><th>#</th><th>Spieler</th><th>Gesamt</th><th>Bereinigt</th><th>Turniere</th><th>Siege</th><th>Freilose</th><th>Niederl.</th><th>Average</th><th>180er</th><th>High Finish</th><th>Streicher</th></tr></thead><tbody>${rows.map((r,i)=>`<tr data-season-player="${esc(r.name)}"><td>${i+1}</td><td><button class="link-btn" data-season-player="${esc(r.name)}">${esc(r.name)}</button></td><td>${r.totalPoints}</td><td><b>${r.cleanPoints}</b></td><td>${r.played}</td><td>${r.wins}</td><td>${r.byes||0}</td><td>${r.losses}</td><td>${r.average??'–'}</td><td>${r.max180}</td><td>${r.checkout}</td><td>${r.dropResults.map(e=>`<span class="drop-pill">${e.present?e.points:0}</span>`).join(' ')||'–'}</td></tr>`).join('')}</tbody></table></div>`;
...
function tournamentCorrectionForm(tournament){
  const matches=(tournament.matches||[]).map((match,index)=>({...match,index})).filter(match=>match.a&&match.b&&match.b!=='Freilos'&&match.sa!==null&&match.sb!==null).sort((a,b)=>(a.round||1)-(b.round||1));
  if(!matches.length)return '<p class="empty-line">Für diesen Spieltag sind keine korrigierbaren Paarungen gespeichert.</p>';
  return `<form class="season-correction-form" data-correction-form="${esc(tournament.id)}"><div class="season-correction-note"><b>Ergebnisse nachträglich korrigieren</b><p>Nur die Spielstände werden geändert. Paarungen und nachfolgende Runden bleiben unverändert; Platzierung und Saisonwertung werden neu berechnet.</p></div><div class="season-correction-matches">${matches.map(match=>`<div class="season-correction-row" data-correction-match="${match.index}"><small>Runde ${match.round||1}</small><b>${esc(tournamentPlayerName(tournament,match.a))}</b><input type="number" min="0" max="30" value="${match.sa}" aria-label="Legs ${esc(tournamentPlayerName(tournament,match.a))}" required><span>:</span><input type="number" min="0" max="30" value="${match.sb}" aria-label="Legs ${esc(tournamentPlayerName(tournament,match.b))}" required><b>${esc(tournamentPlayerName(tournament,match.b))}</b></div>`).join('')}</div><div class="season-correction-actions"><button class="primary" type="submit">KORREKTUR SPEICHERN</button><button class="secondary" type="button" data-cancel-correction="${esc(tournament.id)}">Abbrechen</button></div></form>`;
}
function recalculateTournamentRecord(tournament,season){
  const rows=standingsFor(tournament.players||[],tournament.matches||[]),pointSystem=season?.pointSystem||defaultPointSystem(),previous=new Map((tournament.results||[]).map(result=>[normalizedPlayerName(result.name),result]));
  tournament.results=(tournament.players||[]).map(name=>{const old=previous.get(normalizedPlayerName(name))||{},wins=tournamentWins(name,tournament.matches||[]),losses=tournamentLosses(name,tournament.matches||[]),byes=tournamentByes(name,tournament.matches||[]),row=rows.find(item=>item.name===name)||{};return{...old,name,profileId:old.profileId||tournament.playerProfileIds?.[name]||memberIdForName(name),wins,losses,byes,rank:(rows.findIndex(item=>item.name===name)+1)||0,legsFor:row.lf||0,legsAgainst:row.la||0,points:pointsForWins(wins,pointSystem)+byes}});
...
function correctSeasonTournamentFromForm(form){
  ...
  if(!confirm(`${changed.length===1?'Diesen Spielstand':'Diese Spielstände'} korrigieren und die Saisonwertung neu berechnen? Die folgenden Paarungen bleiben unverändert.`))return;
  changed.forEach(change=>{tournament.matches[change.index].sa=change.sa;tournament.matches[change.index].sb=change.sb});recalculateTournamentRecord(tournament,season);season.stats=calculateSeasonStatisticsSummary(season);
  const history=loadTournamentHistory(),historyRecord=history.find(item=>String(item.id)===String(tournament.id));if(historyRecord){historyRecord.matches=(tournament.matches||[]).map(match=>({...match}));recalculateTournamentRecord(historyRecord,season);localStorage.setItem(TOURNAMENT_HISTORY_KEY,JSON.stringify(history))}
  if(String(state.seasonTournamentId||state.savedToHistory)===String(tournament.id)){state.matches=(tournament.matches||[]).map(match=>({...match}));save()}
...
function renderPlayerSeasonDetail(name){
  ...
  $('#seasonPlayerDetail').innerHTML=`<div class="card player-detail"><button class="secondary close-detail">Schließen</button><div class="player-profile-head"><span class="profile-avatar player-profile-avatar">${avatar}</span><div><span class="eyebrow">SPIELERPROFIL</span><h3>${esc(row.name)}</h3>${fullName}</div></div><div class="season-cards player-profile-stats"><article><span>Turniere</span><b>${row.played}</b></article><article><span>Siege</span><b>${row.wins}</b></article><article><span>Average</span><b>${row.average??'–'}</b></article><article><span>180er</span><b>${row.max180}</b></article><article><span>High Finish</span><b>${row.checkout}</b></article><article><span>Teilnahme</span><b>${Math.round(row.participation*100)}%</b></article></div><div class="table-wrap"><table><thead><tr><th>Datum</th><th>Turnier</th><th>Status</th><th>S/N</th><th>Punkte</th><th>Average</th><th>180er</th><th>Checkout</th></tr></thead><tbody>${row.entries.map(entry=>`<tr class="${entry.dropped?'dropped-row':''}"><td>${entry.date}</td><td>${esc(entry.name)}</td><td>${entry.present?'Gespielt':'Fehltermin'}${entry.dropped?' · gestrichen':''}</td><td>${entry.wins}/${entry.losses}</td><td>${entry.points}</td><td>${entry.average??'–'}</td><td>${entry.max180}</td><td>${entry.checkout}</td></tr>`).join('')}</tbody></table></div></div>`;$('#seasonPlayerDetail').scrollIntoView({behavior:'smooth',block:'start'});
}
...
function publicPlayerProfileHtml(player){
  ...
  const allEntries=player.matches.flatMap(({season,row})=>row.entries.filter(entry=>entry.present).map(entry=>({...entry,seasonName:season.name}))),recent=[...allEntries].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).slice(0,6),played=allEntries.length,wins=allEntries.reduce((sum,item)=>sum+(item.wins||0),0),losses=allEntries.reduce((sum,item)=>sum+(item.losses||0),0),max180=allEntries.reduce((sum,item)=>sum+(item.max180||0),0),checkout=Math.max(0,...allEntries.map(item=>item.checkout||0)),podiums=allEntries.filter(item=>item.rank&&item.rank<=3).length,winRate=wins+losses?Math.round(wins/(wins+losses)*100):0,statEntries=allEntries.filter(item=>item.average!=null),statDarts=statEntries.reduce((sum,item)=>sum+(item.darts||0),0),average=statEntries.length?Number((statDarts?statEntries.reduce((sum,item)=>sum+item.average*(item.darts||0),0)/statDarts:statEntries.reduce((sum,item)=>sum+item.average,0)/statEntries.length).toFixed(1)):null;
  ...
  return `<div class="public-player-back"><button class="secondary" type="button" data-profile-back>← Zurück</button></div><header class="public-player-hero"><span class="profile-avatar public-player-avatar">${avatar}</span><div><span class="eyebrow">TRIPLE20 · SPIELERPROFIL</span><h1 id="playerProfileTitle">${esc(player.name)}</h1><p>${esc(appSettings.club.name||'Dartclub Achensee')} · Persönliche Dartstatistik</p></div><button class="primary public-player-share" type="button" data-profile-share="${esc(player.reference)}">PROFIL TEILEN <span>↗</span></button></header><div class="public-player-stats"><article><small>Turniere</small><b>${played}</b></article><article><small>Siege</small><b>${wins}</b></article><article><small>Average</small><b>${average??'–'}</b></article><article><small>Siegquote</small><b>${winRate}%</b></article><article><small>Podestplätze</small><b>${podiums}</b></article><article><small>180er</small><b>${max180}</b></article><article><small>High Finish</small><b>${checkout||'–'}</b></article></div><div class="public-player-layout"><section><div class="public-player-heading"><span>FORM</span><h2>Letzte Ergebnisse</h2></div>${recent.length?`<div class="public-player-results">${recent.map(item=>`<article><time>${publicDate(item.date)}</time><div><b>${esc(item.name||'Spieltag')}</b><small>${esc(item.seasonName)}${item.average!=null?` · Ø ${item.average}`:''}</small></div><strong>${item.rank?`${item.rank}. Platz`:''}${item.rank&&item.points?' · ':''}${item.points?`${item.points} Pkt.`:''}</strong></article>`).join('')}</div>`:'<p class="public-empty">Noch keine Ergebnisse vorhanden.</p>'}</section><section><div class="public-player-heading"><span>SAISONEN</span><h2>Ranglisten</h2></div><div class="public-player-seasons">${seasons}</div></section></div>`;
}
...
function publicTournamentRecords(){
  ...
  const seasonRecords=(seasonStore.seasons||[]).flatMap(season=>(season.tournaments||[]).filter(tournament=>!tournament.planned||(!completedScheduleIds.has(tournament.id)&&!activeScheduleIds.has(tournament.id))).map(tournament=>({...tournament,seasonName:season.name,seasonId:season.id})));
...
function collectStatsStationMatches(){
  ensureTournamentDayState();return ['men','women'].flatMap(key=>{const tournament=key===state.activeCompetition?competitionSnapshot(state):competitionSnapshot(state.competitions?.[key]||emptyCompetition());if(!tournament.started)return[];return(tournament.matches||[]).map((match,index)=>({key,index,match,tournament})).filter(item=>(item.match.sa===null||item.match.autodartsStats)&&item.match.b!=='Freilos'&&item.match.a&&item.match.b&&!['Noch offen','Sieger aus Vorrunde','Teilnehmer aus Vorrunde'].includes(item.match.a)&&!['Noch offen','Sieger aus Vorrunde','Teilnehmer aus Vorrunde'].includes(item.match.b))});
}
...
function renderStatsStation(preserveResult=false){
  ...
  renderStatsStationAuthorization();renderStatsStationStored();if(!preserveResult){if(preview)preview.classList.add('hidden');if(status)status.textContent=statsStationMatches.length?'Wähle die passende Paarung und anschließend den Autodarts-Screenshot aus.':'Derzeit ist kein laufendes Spiel verfügbar.'}
}
...
async function saveStatsStationResult(form){
  ...
  if(isAdmin()){
    match.autodartsStats=stationStats;save();if(!await publishLiveTournament({notifyOnError:true}))return;
  }else{
    const client=requireSupabaseClient(),{error}=await client.rpc('triple20_save_station_stats',{station_token:statsStationToken(),competition_key:pending.key,match_index:pending.index,match_node_id:pending.nodeId||'',player_a:pending.a,player_b:pending.b,match_round:pending.round||1,station_stats:stationStats});
    if(error){if(/nicht.*freigegeben|invalid/i.test(error.message||''))localStorage.removeItem(STATS_STATION_TOKEN_KEY);$('#statsStationStatus').textContent=`Speichern fehlgeschlagen: ${error.message||'Gerätefreigabe ungültig.'}`;renderStatsStationAuthorization();return}
    match.autodartsStats=stationStats;
  }
  form.querySelector('.stats-station-save')?.setAttribute('disabled','');$('#statsStationStatus').textContent=`Statistik für ${match.a} gegen ${match.b} wurde gespeichert.`;form.insertAdjacentHTML('beforeend','<p class="login-success">✓ Werte dauerhaft beim Spiel gespeichert</p>');renderStatsStationStored();
}
...
$('#seasonTournaments').addEventListener('click',e=>{
  const detailId=e.target.dataset.tournamentDetail,editId=e.target.dataset.editTournament,renameId=e.target.dataset.renameTournament,deleteId=e.target.dataset.deleteTournament,cancelId=e.target.dataset.cancelCorrection;
  ...
  if(editId){editingSeasonTournamentId=String(editId);expandedSeasonTournamentIds.add(String(editId));renderSeasonTournaments();return}
  if(cancelId){editingSeasonTournamentId='';renderSeasonTournaments();return}
