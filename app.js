...
function renderStatsStation(preserveResult=false){
  ...
  renderStatsStationStored();if(!preserveResult){if(preview)preview.classList.add('hidden');if(status)status.textContent=statsStationMatches.length?'Wähle die passende Paarung und anschließend den Autodarts-Screenshot aus.':'Derzeit ist kein laufendes Spiel verfügbar.'}
}
...
async function saveStatsStationResult(form){
  if(!isAdmin()||!statsStationPending)return;
  ...
  match.autodartsStats={a:read(0),b:read(1),capturedAt:new Date().toISOString(),source:'autodarts-screenshot-ocr',confirmedBy:T20Cloud.user?.id||''};
  save();await publishLiveTournament({notifyOnError:true});form.querySelector('.stats-station-save')?.setAttribute('disabled','');$('#statsStationStatus').textContent=`Statistik für ${match.a} gegen ${match.b} wurde gespeichert.`;form.insertAdjacentHTML('beforeend','<p class="login-success">✓ Werte dauerhaft beim Spiel gespeichert</p>');renderStatsStationStored();
}
