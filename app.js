...
function renderSeasonStandings(season=selectedSeason(),rows=calculateSeasonStandings(season)){
  ...
  const desktop=`<div class="table-wrap season-desktop-ranking"><table><thead><tr><th>#</th><th>Spieler</th><th>Gesamt</th><th>Nach Streicher</th><th>Turniere</th><th>Siege</th><th>Freilose</th><th>Niederl.</th><th>Average</th><th>180er</th><th>High Finish</th><th>Streicher</th></tr></thead><tbody>${rows.map((r,i)=>`<tr data-season-player="${esc(r.name)}"><td>${i+1}</td><td><button class="link-btn" data-season-player="${esc(r.name)}">${esc(r.name)}</button></td><td>${seasonRankingMode==='total'?`<b>${r.totalPoints}</b>`:r.totalPoints}</td><td>${seasonRankingMode==='clean'?`<b>${r.cleanPoints}</b>`:r.cleanPoints}</td><td>${r.played}</td><td>${r.wins}</td><td>${r.byes||0}</td><td>${r.losses}</td><td>${r.average??'–'}</td><td>${r.max180}</td><td>${r.checkout}</td><td>${r.dropResults.map(e=>`<span class="drop-pill">${e.present?e.points:0}</span>`).join(' ')||'–'}</td></tr>`).join('')}</tbody></table></div>`;
