...
const COMPETITION_KEYS=['players','playerProfileIds','started','matches','settings','groups','withdrawn','endedEarly','savedToHistory','seasonImportedTo','seasonTournamentId','groupStage','scoreAudit','scoreUndoStack'];
...
function publicEventRow(tournament,status){
  ...
  const key=esc(tournament._publicKey||tournament.id||''),share=status==='future'?`<div class="public-event-share"><button class="secondary" type="button" data-public-calendar="${key}" aria-label="${esc(tournament.name||'Termin')} zum Kalender hinzufügen">Kalender</button><button class="secondary whatsapp-share" type="button" data-public-whatsapp="${key}" aria-label="${esc(tournament.name||'Termin')} über WhatsApp teilen">WhatsApp</button></div>`:'',remove=isAdmin()?`<button class="danger public-delete-event" type="button" data-public-delete="${key}">Löschen</button>`:'';
...
function hideMainSections(){['publicHomeSection','dashboardSection','authSection','settingsSection','seasonSection','shopSection','tournamentSubnav','competitionNav','memberLiveEmpty','setupSection','tournamentSection'].forEach(id=>$('#'+id)?.classList.add('hidden'))}
...
$('#seasonStandings').addEventListener('click',e=>{const toggle=e.target.closest('[data-season-mobile-toggle]');if(toggle){const name=toggle.dataset.seasonMobileToggle,details=[...document.querySelectorAll('[data-season-mobile-details]')].find(item=>item.dataset.seasonMobileDetails===name),open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));toggle.closest('.season-mobile-player')?.classList.toggle('is-open',!open);details?.classList.toggle('hidden',open);return}const el=e.target.closest('[data-season-player]');if(el)renderPlayerSeasonDetail(el.dataset.seasonPlayer)});
