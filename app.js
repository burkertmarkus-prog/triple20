...
window.T20Cloud={
  ...
  startPolling(){clearInterval(this.pollTimer);clearInterval(this.memberPollTimer);clearInterval(this.stationPollTimer);this.pollTimer=setInterval(()=>{if(document.visibilityState==='hidden'||!$('#tvSection')?.classList.contains('hidden'))return;this.loadCloud()},30000);this.memberPollTimer=setInterval(()=>{if(!this.user||document.visibilityState==='hidden'||!$('#tvSection')?.classList.contains('hidden'))return;this.loadPublicMembers().then(()=>{if(!$('#seasonSection')?.classList.contains('hidden'))renderSeasonView()}).catch(error=>console.warn('Mitgliedsnamen konnten nicht aktualisiert werden',error))},120000);this.stationPollTimer=setInterval(()=>{if(state.started||Object.values(state.competitions||{}).some(item=>item?.started))this.loadStationSuggestions()},10000)}
};
