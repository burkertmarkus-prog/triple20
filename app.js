...
function stationMetricInput(key,label,values,{max=999,step='1'}={}){return `<div><span>${label}</span>${values.map((value,side)=>`<input type="number" min="0" max="${max}" step="${step}" data-station-stat="${key}" data-station-side="${side}" value="${/^\d+(?:\.\d+)?$/.test(value)?esc(value):''}" placeholder="–" aria-label="${esc(label)} Spieler ${side+1}">`).join('')}</div>`}
...
function stationNumber(form,key,side){const raw=form.querySelector(`[data-station-stat="${key}"][data-station-side="${side}"]`)?.value;if(raw==='')return null;const number=Number(raw);return Number.isFinite(number)?number:null}
async function saveStatsStationResult(form){
  ...
  const read=side=>({average:stationNumber(form,'average',side),checkoutRate:stationNumber(form,'checkoutRate',side),first9:stationNumber(form,'first9',side),until170:stationNumber(form,'until170',side),highestFinish:stationNumber(form,'highestFinish',side),darts:stationNumber(form,'darts',side),max180:stationNumber(form,'max180',side)});
  ...
  save();await publishLiveTournament({notifyOnError:true});form.querySelector('.stats-station-save')?.setAttribute('disabled','');$('#statsStationStatus').textContent=`Statistik für ${match.a} gegen ${match.b} wurde gespeichert.`;form.insertAdjacentHTML('beforeend','<p class="login-success">✓ Werte dauerhaft beim Spiel gespeichert</p>');
}
