...
function renderMemberSuggestions(){const list=$('#memberSuggestions'),season=selectedSeason();if(!list)return;const empty='<option value="">Mitglied auswählen …</option>';if(!T20Cloud.user){list.innerHTML=empty;return}const entered=new Set(state.players.map(p=>p.toLowerCase())),seasonNames=seasonMembers(season).map(storedName=>currentMemberNickname(season?.memberProfileIds?.[storedName],storedName)),profileNames=registeredMemberProfiles().map(profile=>profile.nickname.trim()),unique=[...new Map([...seasonNames,...profileNames].filter(Boolean).map(name=>[name.toLowerCase(),name])).values()].sort((a,b)=>a.localeCompare(b,'de'));list.innerHTML=empty+unique.filter(name=>!entered.has(name.toLowerCase())).map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join('')}
...
function showTournament(updateUrl=true){hideMainSections();$('#competitionNav')?.classList.remove('hidden');renderTournament();renderNavigation();if(isAdmin())T20Cloud.loadStationSuggestions().catch(error=>console.warn('Ergebnisvorschläge konnten nicht geladen werden:',error));if(updateUrl)updateAppUrl('turnier')}
