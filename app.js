...
async function saveStatsStationResult(form){
  ...
  if(isAdmin()){
    match.autodartsStats=stationStats;save();if(!await publishLiveTournament({notifyOnError:true})){if(saveButton){saveButton.disabled=false;saveButton.innerHTML='ERNEUT VERSUCHEN <span>→</span>'}return}
  }else{
    const client=requireSupabaseClient(),{data,error}=await client.rpc('triple20_save_station_stats',{station_token:statsStationToken(),competition_key:pending.key,match_index:pending.index,match_node_id:pending.nodeId||'',player_a:pending.a,player_b:pending.b,match_round:pending.round||1,station_stats:stationStats});
  ...
  if(saveButton){saveButton.disabled=false;saveButton.innerHTML='WERTE ERNEUT SPEICHERN <span>↻</span>'}$('#statsStationStatus').textContent=`Cloud bestätigt: Statistik für ${match.a} gegen ${match.b} ist gespeichert.`;form.insertAdjacentHTML('beforeend','<p class="login-success stats-station-save-confirmed">✓ Von der Cloud bestätigt und dauerhaft beim Spiel gespeichert</p>');renderStatsStationStored();
}
